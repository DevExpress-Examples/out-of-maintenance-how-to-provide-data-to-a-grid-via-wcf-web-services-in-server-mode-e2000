...
Namespace DXGridSample
    ...
    Partial Public Class Window1
        ...
        Public Sub New()
            ...
            Dim collectionSource As New LinqServerModeSource() With {.ElementType = GetType(WpfServerSideGridTest), _
                .QueryableSource = simpleExampleContext.Objects, .KeyExpression = "OID"}
            grid.DataSource = collectionSource

            infoContainer.DataContext = wrapper
            scrollViewer.ScrollToEnd()
            logHeaderText.Text = "ServerProcessingTime(ms)" & Constants.vbTab & "TotalProcessingTime(ms)" & _
            Constants.vbTab & "RequestSize(bytes)" & Constants.vbTab & "ResponseSize(bytes)"
        End Sub
    End Class
    ...
    Public Class QueryRemoteHandlerWrapper
        ...
        Public Shared ReadOnly TotalBytesSentProperty As DependencyProperty = _
        DependencyProperty.Register("TotalBytesSent", GetType(Integer), GetType(QueryRemoteHandlerWrapper), _
        ...
        Public Shared ReadOnly TotalBytesReceivedProperty As DependencyProperty = _
                DependencyProperty.Register("TotalBytesReceived", GetType(Integer), _
                                            GetType(QueryRemoteHandlerWrapper), New UIPropertyMetadata(0))
        ...
        Public Property NotInServerMode() As Boolean
            ...
            End Set
        End Property
        Public Shared ReadOnly NotInServerModeProperty As DependencyProperty = _
                DependencyProperty.Register("NotInServerMode", GetType(Boolean), GetType(QueryRemoteHandlerWrapper), _
        ...
        Public Sub New(ByVal handler As IQueryRemoteHandler)
            AddHandler GZipMessageEncoderFactory.MessageReceived, _
            AddressOf GZipMessageEncoderFactory_MessageReceived
            AddHandler GZipMessageEncoderFactory.MessageSent, _
            AddressOf GZipMessageEncoderFactory_MessageSent
        ...
        Private Sub GZipMessageEncoderFactory_MessageSent(ByVal sender As Object, _
                                                          ByVal e As MessageSizeEventArgs)
            currentInfo.RequestSize = e.MessageSize
        End Sub

        Private Sub GZipMessageEncoderFactory_MessageReceived(ByVal sender As Object, _
                                                              ByVal e As MessageSizeEventArgs)
...
#Region "IQueryRemoteHandler Members"
        ...
        Public Function Retrieve(ByVal expression As InterLinq.Expressions.SerializableExpression) As Object _
        Implements IQueryRemoteHandler.Retrieve
        ...
        Private Sub RefreshLog(ByVal serverProcessingTime As Integer, ByVal totalProcessingTime As Integer, ByVal isServerMode As Boolean)
            Dim logEntry As String = String.Format("{0:### ##0}" & Constants.vbTab + Constants.vbTab + Constants.vbTab + _
                Constants.vbTab & "{1:### ##0}" & Constants.vbTab + Constants.vbTab + Constants.vbTab + Constants.vbTab _
                & "{2:### ##0}" & Constants.vbTab + Constants.vbTab + Constants.vbTab & "{3:### ##0}", serverProcessingTime, _
                totalProcessingTime, currentInfo.RequestSize, currentInfo.ResponseSize)
